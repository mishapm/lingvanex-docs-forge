export const referenceDocumentation: Record<string, string> = {
  '/getting-languages': `# Get languages
...
<ApiEndpoint method="POST" url="https://api-gl.lingvanex.com/language/translate/v2/languages" />
...
<div class="space-y-4 my-4">
  <div class="border border-border rounded-lg p-4 bg-card">
    ...
    <div class="ml-0 space-y-2">
      <p class="text-sm text-muted-foreground">Type: <code class="text-xs">string</code></p>
      <p class="text-sm text-foreground mt-2">The language code in the "language code" format from which the text is translated. The language code is represented only in lowercase letters (en, de, es).</p>
    </div>
  </div>
</div>
