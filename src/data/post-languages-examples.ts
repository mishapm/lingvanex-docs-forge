export const postLanguagesExamples = [
  {
    language: 'Shell',
  ...
  {
    language: 'C++',
    request: '```cpp\n#include <iostream>\n#include <curl/curl.h>\n#include <string>\n\nint main() {\n    CURL *curl;\n    CURLcode res;\n    \n    curl_global_init(CURL_GLOBAL_DEFAULT);\n    curl = curl_easy_init();\n    \n    if(curl) {\n        curl_easy_setopt(curl, CURLOPT_URL, "https://api-gl.lingvanex.com/language/translate/v2/languages");\n        \n        struct curl_slist *headers = NULL;\n        headers = curl_slist_append(headers, "Authorization: Bearer YOUR_API_KEY");\n        headers = curl_slist_append(headers, "Content-Type: application/json");\n        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);\n        \n        std::string data = "{\\"target\\":\\"en\\"}";\n        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, data.c_str());\n        \n        res = curl_easy_perform(curl);\n        \n        if(res != CURLE_OK)\n            fprintf(stderr, "curl_easy_perform() failed: %s\\n", curl_easy_strerror(res));\n        \n        curl_easy_cleanup(curl);\n    }\n    \n    curl_global_cleanup();\n    return 0;\n}\n```',
    response: '```json\n{\n  "result": [\n    {\n      "code": "af",\n      "name": "Afrikaans"\n    },\n    {\n      "code": "ar",\n      "name": "Arabic"\n    },\n    {\n      "code": "zh",\n      "name": "Chinese"\n    }\n  ]\n}\n```'
  },
  {
    language: 'Clojure',
    request: '```clojure\n(require \'[clj-http.client :as client])\n(require \'[cheshire.core :as json])\n\n(def response\n  (client/post "https://api-gl.lingvanex.com/language/translate/v2/languages"\n    {:headers {"Authorization" "Bearer YOUR_API_KEY"\n               "Content-Type" "application/json"}\n     :body (json/generate-string {:target "en"})}))\n\n(println (:body response))\n```',
    response: '```json\n{\n  "result": [\n    {\n      "code": "af",\n      "name": "Afrikaans"\n    },\n    {\n      "code": "ar",\n      "name": "Arabic"\n    },\n    {\n      "code": "zh",\n      "name": "Chinese"\n    }\n  ]\n}\n```'
  },
  {
    language: 'Go',
