...
export function LanguageCodeExamples({
  examples
}: LanguageCodeExamplesProps) {
  ...
  return <div className="mt-6">
      <div className="mb-4">
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-48 bg-background border border-border shadow-lg z-50">
            {examples.map(example => <DropdownMenuItem key={example.language} onClick={() => setSelectedLanguage(example.language)} className="cursor-pointer hover:bg-muted focus:bg-muted">
                {example.language}
              </DropdownMenuItem>)}
          </DropdownMenuContent>
        </DropdownMenu>
      ...
      {currentExample && <div className="space-y-6">
          <div>
